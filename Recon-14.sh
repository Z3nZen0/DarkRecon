...
function option1() 
{
  ...
  # Create a folder for the output files using the domain name
  mkdir -p $folder_path
  ...
  httpx -title -tech-detect -status-code -ip -asn -t 75 -rl 100 -o "$folder_path/HTTPX_$domain.txt"
  ...
  echo "Web App Test completed for $url"
}
