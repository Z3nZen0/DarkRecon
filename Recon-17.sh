...
function option1() 
{
  ...
  cd ~/go/bin
  ./subfinder -d "$domain" -nW -oI -o "$folder_path/$domain/Subfinder_$domain.txt"
  echo "Running httpx to perform HTTP enumeration..."
  ./httpx -u "$domain" -title -tech-detect -status-code -ip -asn -rl 100 -o "$folder_path/$domain/HTTPX_$domain.txt"
  ...
  sudo nmap -sC -sV -sS -T3 --top-ports 500 --open "$url" -o "$folder_path/$domain/NMAP_$domain.txt"
  echo "Running wafw00f to detect WAF presence..."
  wafw00f http://"$url" -a -r -o "$folder_path/$domain/wafw00f_http_$domain.txt"
  wafw00f https://"$url" -a -r -o "$folder_path/$domain/wafw00f_https_$domain.txt"
  ...
  cat "$folder_path/$domain/Param_$domain.txt" | ./gf sqli | tee -a "$folder_path/$domain/sqli.txt"
  echo "Running dalfox to test for XSS vulnerabilities..."
  ./dalfox file "$folder_path/$domain/xss.txt" --remote-payloads=payloadbox,portswigger --waf-evasion --report -o "$folder_path/$domain/Dalfox_$domain.txt"
  ...
  sqlmap -u "$folder_path/$domain/sqli.txt" --random-agent --batch --level=5 --risk=3 --current-user --current-db --banner --passwords --threads=4 --output-dir="$folder_path/$domain"
  echo "Running arjun to collect parameters..."
  cd ~/WebApp_Tools/arjun
  arjun -u "$url" -t 20 -d 2 -c 250 --stable -o "$folder_path/$domain/Arjun_$domain.txt"
  cd ~/go/bin
  cat "$folder_path/$domain/Arjun_$domain.txt" | ./gf redirect | tee -a "$folder_path/redirect_Arjun.txt"
  cat "$folder_path/$domain/Arjun_$domain.txt" | ./gf xss | tee -a "$folder_path/xss_Arjun.txt"
  cat "$folder_path/$domain/Arjun_$domain.txt" | ./gf idor | tee -a "$folder_path/idor_Arjun.txt"
  cat "$folder_path/$domain/Arjun_$domain.txt" | ./gf ssrf | tee -a "$folder_path/ssrf_Arjun.txt"
  cat "$folder_path/$domain/Arjun_$domain.txt" | ./gf wordpress | tee -a "$folder_path/WordPress_Arjun.txt"
  cat "$folder_path/$domain/Arjun_$domain.txt" | ./gf sqli | tee -a "$folder_path/sqli_Arjun.txt"
  cd ~/Web_Tools
  echo "Running Gobuster to bruteforce Directories..."
  gobuster dir -u $url -t 20 -q -r --random-agent -w ~/tools/SecLists/Discovery/Web-Content/directory-list-2.3-big.txt -o "$folder_path/$domain/Dirb.txt"
  echo "Web App Test completed for $url in the '$project_name' project folder: $folder_path/$domain"

  # Generate the report
  echo "Generating the report..."
  python generate_report.py "$folder_path/$domain"
}
...
# Based on the user's selection, it runs the chosen tool(s) with appropriate arguments.
  ...
  for tool in "${tools[@]}"; do
    case $tool in
      ...
      4)
        cd ~/go/bin
        ./nuclei -u "$url" -as -fhr -rl 150 -o "$folder_path/Nuclei_$domain.txt"
        ;;
      5)
        sudo nmap -sC -sV -sS -T3 --top-ports 500 --open "$url" -o "$folder_path/NMAP_$domain.txt"
        ;;
      6)
        wafw00f "$url" -a -r -o "$folder_path/wafw00f_$domain.txt"
        ;;
      7)
        cd ~/WebApp_Tools
        ...
        cat "$folder_path/Param_$domain.txt" | ./gf sqli | tee -a "$folder_path/$domain/sqli.txt"
        ;;
      8)
        cd ~/WebApp_Tools
        ...
        cat "$folder_path/Arjun_$domain.txt" | ./gf sqli | tee -a "$folder_path/sqli_Arjun.txt"
        ;;
      9)
        cd ~/go/bin      
      ...
      12)
        cd ~/WebApp_Tools
        amass enum -u $url -brute -max-depth 5 -o "$folder_path/Amass_$domain.txt" -d $url
        ;;
      *)
